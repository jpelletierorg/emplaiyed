...
Handles application submission on Ashby-powered job boards (jobs.ashbyhq.com).
"""
...
class AshbyAdapter(BaseAdapter):
    """Handles Ashby ATS application forms."""

    async def discover_fields(self, session: BrowserSession) -> list[dict]:
        """Discover fields on an Ashby application form."""
        ...
        () => {
            ...
            for (const el of inputs) {
                ...
                if (!label && el.closest('[class*="FormField"]')) {
                    const labelEl = el.closest('[class*="FormField"]').querySelector('label');
                    if (labelEl) label = labelEl.innerText.trim();
                }
                if (!label) {
        ...
        """)
        logger.info("Ashby: discovered %d fields", len(fields))
        return fields
    ...
    async def fill_and_submit(
        ...
        *,
        ...
        if plan.unmapped_required:
            logger.warning(
                "Ashby: unmapped required fields: %s", plan.unmapped_required
            )
        ...
        for mapping in plan.text_fields:
            try:
                ...
                logger.warning("Ashby: failed to fill %s: %s", mapping.selector, exc)
        ...
        for mapping in plan.file_uploads:
            try:
                ...
                logger.warning(
                    "Ashby: failed to upload to %s: %s", mapping.selector, exc
                )
